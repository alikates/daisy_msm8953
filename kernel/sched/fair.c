...
static int select_energy_cpu_brute(struct task_struct *p, int prev_cpu, int sibling_count_hint)
{
		    ...
		    int sibling_count_hint)
{
	...
	if (energy_aware()) {
		rcu_read_lock();
		new_cpu = select_energy_cpu_brute(p, prev_cpu, sibling_count_hint);
		rcu_read_unlock();
		return new_cpu;
	}
...
void check_for_migration(struct rq *rq, struct task_struct *p)
{
	...
	if (rq->misfit_task) {
		...
		rcu_read_lock();
		new_cpu = select_energy_cpu_brute(p, cpu, 1);
		rcu_read_unlock();
